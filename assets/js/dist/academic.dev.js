...
(function ($) {
  ...
  function getNavBarHeight() {
    ...
    var navbar_offset = $navbar.outerHeight();
  ...
  function scrollToAnchor(target) {
    ...
    target = typeof target === 'undefined' || _typeof(target) === 'object' ? decodeURIComponent(window.location.hash) : target; // If target element exists, scroll to it taking into account fixed navigation bar offset.
    ...
    if ($(target).length) {
      ...
      var elementOffset = Math.ceil($(target).offset().top - getNavBarHeight()); // Round up to highlight right ID!
  ...
  function removeQueryParamsFromUrl() {
    if (window.history.replaceState) {
      var urlWithoutSearchParams = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.hash;
  ...
  $('#navbar-main li.nav-item a.nav-link').on('click', function (event) {
    ...
    var hash = this.hash; // If we are on a widget page and the navbar link is to a section on the same page.

    if (this.pathname === window.location.pathname && hash && $(hash).length && $(".js-widget-page").length > 0) {
      // Prevent default click behavior.
      event.preventDefault(); // Use jQuery's animate() method for smooth page scrolling.
      // The numerical parameter specifies the time (ms) taken to scroll to the specified hash.

      var elementOffset = Math.ceil($(hash).offset().top - getNavBarHeight()); // Round up to highlight right ID!
      // Uncomment to debug.
  ...
  var pubFilters = {}; // Search term.

  var searchRegex; // Filter values (concatenated).

  var filterValues; // Publication container.

  var $grid_pubs = $('#container-publications'); // Initialise Isotope.
  ...
  $('.pub-filters').on('change', function () {
    ...
    var filterGroup = $this[0].getAttribute('data-filter-group'); // Set filter for group.

    pubFilters[filterGroup] = this.value; // Combine filters.

    filterValues = concatValues(pubFilters); // Activate filters.

    $grid_pubs.isotope(); // If filtering by publication type, update the URL hash to enable direct linking to results.
  ...
  function filter_publications() {
    ...
    var filterValue = '*'; // Check if hash is numeric.
    ...
    $grid_pubs.isotope(); // Set selected option.
  ...
  * --------------------------------------------------------------------------- */
  ...
  function toggleSearchDialog() {
    if ($('body').hasClass('searching')) {
      ...
      $('body').removeClass('searching compensate-for-scrollbar'); // Remove search query params from URL as user has finished searching.

      removeQueryParamsFromUrl(); // Prevent fixed positioned elements (e.g. navbar) moving due to scrollbars.
  ...
  * --------------------------------------------------------------------------- */
  ...
  function changeThemeModeClick() {
    ...
    var $themeChanger = $('.js-dark-toggle i');
  ...
  function renderThemeVariation(isDarkTheme) {
    ...
    var diagramEnabled = $('script[title=mermaid]').length > 0; // Check if re-render required.
  ...
  function initThemeVariation() {
    ...
    var isDarkTheme = getThemeVariation();
  ...
  * --------------------------------------------------------------------------- */
  ...
  function normalizeCarouselSlideHeights() {
    $('.carousel').each(function () {
      // Get carousel slides.
      var items = $('.carousel-item', this); // Reset all slide heights.

      items.css('min-height', 0); // Normalize all slide heights.
  ...
  * --------------------------------------------------------------------------- */
  ...
  function fixHugoOutput() {
    ...
    $('#TableOfContents li a').addClass('nav-link'); // Fix Goldmark task lists (remove bullet points).
  ...
  $(document).ready(function () {
    fixHugoOutput();
    fixMermaid(); // Initialise code highlighting if enabled for this page.
    ...
    initThemeVariation(); // Change theme mode.
    ...
    darkModeMediaQuery.addListener(function (e) {
      ...
      console.log("OS dark mode preference changed to ".concat(darkModeOn ? '🌒 on' : '☀️ off', "."));
      var currentThemeVariation = parseInt(localStorage.getItem('dark_mode') || 2);
      var isDarkTheme;
  ...
  $(window).on('load', function () {
    ...
    if ($('.pub-filters-select')) {
      filter_publications(); // Useful for changing hash manually (e.g. in development):
    ...
    $('.js-copy-cite').click(function (e) {
      e.preventDefault(); // Get selection.
    ...
    initMap(); // Print latest version of GitHub projects.

    var githubReleaseSelector = '.js-github-release';
    if ($(githubReleaseSelector).length > 0) printLatestRelease(githubReleaseSelector, $(githubReleaseSelector).data('repo')); // On search icon click toggle search dialog.
  ...
  $(window).on('load resize orientationchange', normalizeCarouselSlideHeights); // Automatic main menu dropdowns on mouse over.
  ...
  $('body').on('mouseenter mouseleave', '.dropdown', function (e) {
    ...
    }, 300); // Re-initialize Scrollspy with dynamic navbar height offset.
